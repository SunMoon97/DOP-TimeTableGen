...
combined_branches_ordered.append(mech_code)  # A4
combined_branches_ordered.append(eee_code)   # A3
combined_branches_ordered.append(ece_code)   # AA
combined_branches_ordered.append(eni_code)     # A8


# Add BXA7 branches
for value in branches.values():
    if value not in {cs_code, che_code,mech_code,eee_code,eni_code,ece_code}:
...
for value in branches.values():
    if value not in {cs_code, che_code,mech_code,eee_code,eni_code,ece_code}:
        combined_branches_ordered.append(f"{value}{mech_code}")  # e.g., B1A4

# Add BXA1 branches
for value in branches.values():
    if value not in {cs_code, che_code,mech_code,eee_code,eni_code,ece_code}:
        combined_branches_ordered.append(f"{value}{eee_code}")  # e.g., B1A4

# Add BXA1 branches
for value in branches.values():
    if value not in {cs_code, che_code,mech_code,eee_code,eni_code,ece_code}:
        combined_branches_ordered.append(f"{value}{eni_code}")  # e.g., B1A4

# Add BXA1 branches
for value in branches.values():
    if value not in {cs_code, che_code,mech_code,eee_code,eni_code,ece_code}:
        combined_branches_ordered.append(f"{value}{ece_code}")  # e.g., B1A4
...
combined_branches[mech_code] = (mech_branch, mech_code)
combined_branches[eee_code] = (eee_branch, eee_code)
combined_branches[ece_code] = (ece_branch, ece_code)
combined_branches[eni_code] = (eni_branch, eni_code)

# Add BXA7 and BXA1
for branch_code in combined_branches_ordered[6:]:
...
course_to_year_branch = {
    row['SEM 1']: (row['Year'], branches.get(row['branch'], che_code))
    for _, row in sheet1_filtered.iterrows()
}
course_to_year_branch = {
    row['SEM 1']: (row['Year'], branches.get(row['branch'], eee_code))
    for _, row in sheet1_filtered.iterrows()
}
course_to_year_branch = {
    row['SEM 1']: (row['Year'], branches.get(row['branch'], ece_code))
    for _, row in sheet1_filtered.iterrows()
}
course_to_year_branch = {
    row['SEM 1']: (row['Year'], branches.get(row['branch'], eni_code))
    for _, row in sheet1_filtered.iterrows()
}
course_to_year_branch = {
    row['SEM 1']: (row['Year'], branches.get(row['branch'], mech_code))
    for _, row in sheet1_filtered.iterrows()
}

# Split the cells containing multiple course codes
sheet2['courseno'] = sheet2['courseno'].str.split('/')

# Explode the DataFrame to create separate rows for each course code
sheet2 = sheet2.explode('courseno')

# Strip any leading/trailing whitespace from course codes
sheet2['courseno'] = sheet2['courseno'].str.strip()

# Match the course codes
...
for _, row in matching_courses.iterrows():
    ...
    for combined_branch in combined_branches_ordered:
        ...
        if base_branch_code == base_code or base_branch_code == target_code:
            if (base_branch_code == target_code and base_branch_code in {cs_code, che_code,mech_code,eee_code,ece_code,eni_code}) and combined_branch not in {cs_code, che_code,mech_code,eee_code,ece_code,eni_code}:
